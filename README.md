...
The app includes Configuration filters and optional real sensor/light access through the local broker. It defaults to demo mode. Native enrollment, NHP admission, NHP-FRP, TLS, signed handoffs and page authorization remain real.
...
Beta.13 refreshes the app into enrollment after a connection reset. During
enrollment, the credential input is replaced by a spinner showing certificate
and startup progress. Failed enrollment restores the input. The owner account
website remains a separate service.
